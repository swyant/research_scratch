...
params[:bessel_polynomial_degree] = [4]
params[:inverse_polynomial_degree] = [10]

# one-body potential
params[:onebody] = [1]

# two-body linear POD potential
params[:twobody_number_radial_basis_functions] = [8]

# three-body linear POD potential
params[:threebody_number_radial_basis_functions] = [6]
params[:threebody_angular_degree] = [2, 3, 4]

# four-body linear POD potential
params[:fourbody_number_radial_basis_functions] = [5]
params[:fourbody_angular_degree] = [2, 3, 4]

params[:true4BodyDesc] = [1]

# five-body linear POD potential
params[:fivebody_number_radial_basis_functions] = [4]
params[:fivebody_angular_degree] = [2, 3, 4]

# six-body linear POD potential
params[:sixbody_number_radial_basis_functions] = [3]
params[:sixbody_angular_degree] = [2, 3, 4]

# seven-body linear POD potential
params[:sevenbody_number_radial_basis_functions] = [0, 2]
params[:sevenbody_angular_degree] = [2, 3, 4]
...
for vs_dat in product(values(data)...)
    ...
    for vs_pars in product(values(params)...)
        curr_params = OrderedDict(ks_pars .=> vs_pars)
        if  curr_params[:twobody_number_radial_basis_functions] >=
            curr_params[:threebody_number_radial_basis_functions] >=
            curr_params[:fourbody_number_radial_basis_functions] >=
            curr_params[:fivebody_number_radial_basis_functions] >=
            curr_params[:sixbody_number_radial_basis_functions] >=
            curr_params[:sevenbody_number_radial_basis_functions]
