...
#struct SharedBasisCommitteePotential{P}
#    basis::P
#    member_coeffs::Vector{Vector{Float64}}
#    leader::Integer
#    energy_units::Unitful.Unitlike
#    length_units::Unitful.Unitlike
#    force_units::Unitful.Unitlike
#
#    #inner constructor to enforce that leader can appropriately access members
#    function SharedBasisCommitteePotential(
#                basis::P
#                member_coeffs::Vector{Vector{Float64}}
#                eunits::Union{NOUNIT_T, Unitful.Units{UE,E_DIM,nothing},Unitful.Units{UE,EN_DIM,nothing}},
#                lunits::Union{NOUNIT_T, Unitful.Units{UL,L_DIM,nothing}}) where {P, UE, UL}
...
#                            energy_units=u"eV",
#                            length_units=u"Å") where {P}
#    ref_basis = members[1].basis
#    for member in members
#        if !(member.basis == ref_basis)
#            error("basis doesn't match amongst members")
#    cmte_pot = CommitteePotential(members,leader,energy_units,length_units)
#    cmte_pot
#end
...
function PotentialLearning.potential_energy(config::PotentialLearning.Configuration,
                                           cmte_pot::CommitteePotential;
                                           neighbors=nothing,
                                           n_threads::Integer=Threads.nthreads())

  poteng = PotentialLearning.potential_energy(config,cmte_pot.members[cmte_pot.leader]) * cmte_pot.energy_units
...
# Quickly hacking this up
function compute_all_energies(config::PotentialLearning.Configuration, cmte_pot::CommitteePotential)
  # This manual type stuff is only needed so long as IP.jl doesn't satistfy the AtomsCalculators interface

  all_potengs = [PotentialLearning.potential_energy(config,pot) * cmte_pot.energy_units
                    for pot in cmte_pot.members]
