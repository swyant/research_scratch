...
data_folder= "../../files/small_lammps_compat_subset"

lmp_pod=LAMMPS_POD("../../files/sample_6body_hfo2_param.pod", [:Hf, :O])
hfo2_lbp = LBasisPotential(lmp_pod,"../../files/ref_small_lammps_compat_HfO2_HIGHP_coefficients.pod")

header=[:config, :num_atoms, :volume, :energy, :DFT_energy, :energy_error, :force, :DFT_force, :force_error]
ddict = read_segmented_files("../../files/ref_small_lammps_compat_HfO2_HIGHP_training_analysis.pod"; header=header, delim=' ', ignorerepeated=true, skipto=2)
