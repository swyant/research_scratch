...
def compute_predictions(configurations: Dict[str, List[Atoms]],
                       calculator: Calculator, E0s=None) -> Dict[str, List[Atoms]]:
    """
    ...
    if E0s is not None:
        ...
        orig_energy_map = dict(zip(model_atomic_numbers,atomic_energies_tensor[:]))
        print(orig_energy_map)
    ...
    for label, atoms_list in configurations.items():
        ...
        for i, atoms in enumerate(atoms_list):
            try:
                ...
                # This is a very fragile, temporary hack. If you call get_potential_energy() again,
                # this will be overwritten
                if E0s is not None:

                    # This was totally wrong
                    #isolated_atoms_energy = atoms_copy.calc.results["node_energy"].sum()
                    ##print(f"isolated atoms node energies: {atoms_copy.calc.results["node_energy"]}")
                    #binding_energy = energy - isolated_atoms_energy
                    ##print(f"Binding energy: {binding_energy}")

                    atomic_numbers = atoms_copy.get_atomic_numbers()
                    isolated_atom_energy = sum([orig_energy_map[z] for z in atomic_numbers])
                    binding_energy = energy - isolated_atom_energy
                    print(f"binding energy {binding_energy}")

                    atom_e0s = [E0s[z] for z in atomic_numbers]
                    print(atom_e0s)
                    print(f"sum of atom E0s: {sum(atom_e0s)}")
                    potential_energy = binding_energy + sum([E0s[z] for z in atomic_numbers])
                    print(f"potential energy {potential_energy}")
                    atoms_copy.info["orig_predicted_energy"] = energy
                    atoms_copy.info["fixed_predicted_energy"] = potential_energy
...
def _safe_get_energy(atoms: Atoms, is_reference: bool = False) -> Optional[float]:
    ...
    try:
        if is_reference:
            ...
            # For predicted data
            if "fixed_predicted_energy" in atoms.info:
                energy = atoms.info["fixed_predicted_energy"]
                return energy / len(atoms)
...
# Also predicted_configs still has REF_energy
def calculate_errors(reference_configs: Dict[str, List[Atoms]],
    ...
    for label in reference_configs.keys():
        ...
        energy_errors = []
        squared_energy_errors = []
        force_errors = []
        squared_force_errors = []
        n_atoms_total = 0
        ...
        for ref_atoms, pred_atoms in zip(ref_atoms_list, pred_atoms_list):
            ...
            if ref_energy is not None and pred_energy is not None:
                ...
                all_energy_errors.append(abs(ref_energy - pred_energy)) # slop, doesn't get used
                squared_energy_errors.append((ref_energy-pred_energy)**2)
            ...
            if ref_forces is not None and pred_forces is not None:
                if ref_forces.shape == pred_forces.shape:
                    ...
                    squared_force_errors.extend(np.square(ref_forces_flat - pred_forces_flat))
        ...
        label_errors[label] = {
            ...
            'energy_rmse': sqrt(np.mean(squared_energy_errors)) if squared_energy_errors else np.nan,
            'force_mae': np.mean(force_errors) if force_errors else np.nan,
            'force_rmse': sqrt(np.mean(squared_force_errors)) if squared_force_errors else np.nan,
