...
function simple_ace_nvt(;driver_yace_fname="sample.yace",
                        temp=300, 
                        vel_seed=12280329, 
                        atom_type_list=["Hf","O"]) 
    atomtype_str = ""
    ...
    command(lmp, "velocity     all create $(temp) $(vel_seed) mom yes rot yes dist gaussian")
    command(lmp, "fix          nvt all nvt temp $(temp) $(temp) 0.1")

    lmp, nothing
end 

function simulate_step!(lmp, startstop=nothing)
    try
        run_command = "run 1"
